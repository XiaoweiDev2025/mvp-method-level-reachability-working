...
> **L1 is independently verified, not inferred from the seed.** A validated seed records which package and version range a CVE affects *in general*; it says nothing about whether the specific project being analysed actually depends on an affected version. `pipeline.py` checks this directly, before static/runtime analysis runs at all, via `component_check.py` reading each supplied JAR's own embedded Maven coordinates (`META-INF/maven/<groupId>/<artifactId>/pom.properties`). See [L1 component-presence check](#l1-component-presence-check) below for the three-way outcome and why an inconclusive check does not short-circuit.
>
...
|--------|---------|----------|----------------|
| *(component confirmed out of vulnerable_range — see [L1 component-presence check](#l1-component-presence-check))* | not run | not_affected_candidate | **L1** |
...
**L1 component-presence check**: before static or runtime analysis runs at all, `pipeline.py` calls `component_check.py::check_component_present()` to check whether the project's own JARs actually carry the seed's vulnerable component at a version inside `vulnerable_range`, by reading each JAR's own embedded Maven coordinates (`META-INF/maven/<groupId>/<artifactId>/pom.properties` — present in essentially every JAR built and published through Maven, including the ones bundled in this repo's own demo projects). This closes a real gap: earlier versions of this pipeline went straight from a validated seed to static analysis, implicitly trusting that whoever supplied `--project-jars` had already confirmed the vulnerable component was actually present at an affected version — `L1_COMPONENT_PRESENT` was defined in the `EvidenceLevel` enum from the start but no code path ever independently produced it. The check has three possible outcomes:
- **Confirmed out of range** — the version found is outside `vulnerable_range`. `fusion.py::fuse_component_absent()` short-circuits directly to an `L1 not_affected_candidate` finding; static/runtime analysis is skipped entirely, since there is no reachability question to ask about a component version the project does not depend on.
- **Confirmed in range** — analysis proceeds exactly as before, through static/runtime analysis and `fusion.py::_decide()`.
- **Inconclusive** (no JAR with matching Maven coordinates found at all — e.g. a non-Maven build, or a shaded JAR with its coordinate metadata stripped) — also proceeds exactly as before. This is deliberate: the absence of `pom.properties` metadata is not the same claim as the absence of the component itself, so an inconclusive check must never be treated as a confident negative.

The version comparator (`_compare_versions`/`_version_satisfies_range`) is a deliberately narrow implementation, not a full Maven `ComparableVersion` algorithm — it does not implement Maven's qualifier-ordering table (`alpha < beta < milestone < rc < snapshot < (release) < sp`), and only the first matching JAR found is checked, so a second, independently-vendored or shaded copy of the same component under different coordinates would not be caught. This is why the L1 multiplier (0.05) is non-zero rather than treating a confirmed-out-of-range result as absolute proof of safety.
